...
from be_skeleton_test.apps.accounts.exceptions import (
    InvalidPasswordError,
    InvalidResetPasswordSignatureError,
    WrongPasswordError,
)
from be_skeleton_test.apps.accounts.models import UserAccount
from be_skeleton_test.apps.accounts.services.password import PasswordService


PASSWORD_SERVICE_PATH = "be_skeleton_test.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.be_skeleton_test_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
